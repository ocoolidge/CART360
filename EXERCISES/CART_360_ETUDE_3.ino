...
void updateTimer()
{
  ...
  timePassed = millis() - startTime;      //marks how much time has passed since timer was started
...
void playCurrentNote()
{
   offsetFrequency = getPhotoFrequency();
    ...
    if(analogRead(NOTE_IN_PIN) > 5){
      tone(BUZZER_PIN, (analogRead(NOTE_IN_PIN) + (getRunningAverage()*4)), duration);  //note played is button input + photocell input
    }
...
{
  int sampleTotal = 0;
  int rawSenseVal = getPhotoFrequency();
...
**************************************************************************/
void colorLED(int col1)
{
  analogWrite(LED_PIN_R, col1 + 100);   //col1 effects led colors in different ways bucause of values added to it
  analogWrite(LED_PIN_G, col1);   
  analogWrite(LED_PIN_B, col1 + 50);   
